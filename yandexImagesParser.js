...
async function downloadImage(url, filepath) {
  const res = await fetch(url); // глобальный fetch в Node 20
  if (!res.ok) {
 ...
 * - city, pointTitle — из твоих данных маршрутов
 * - limit — сколько картинок парсить
 */
export async function runParser({
  ...
  const tmpDir = path.join(__dirname, 'tmp', routeId, String(pointIndex));
